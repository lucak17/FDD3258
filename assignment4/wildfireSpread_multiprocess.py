...
def simulate_wildfire(dummy):
    ...
    return np.array(fire_spread + [0] * (DAYS - len(fire_spread)))
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=numSimulations) as pool:
        resultsPool = pool.map(simulate_wildfire,[dummy] * numSimulations)
    
    mean_res = np.mean(resultsPool,0)
    std_res = np.std(resultsPool,0)
    ...
    fig = plt.plot(range(len(mean_res)), mean_res, label="Burning Trees")
    ...
    plt.legend()
    plt.savefig("wildFireMultiprocess.pdf", format='pdf',bbox_inches="tight", transparent=False)
    plt.show()